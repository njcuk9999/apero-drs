...
def generate_finder_charts(
    ...
    try:
        gaia_sources = _get_gaia_sources(
            obs_coords, obs_time, fcfg["radius"]["G"], fcfg["max_pm"]
        )
    ...
    for band in bands:
        ...
        image, wcs = _seed_image(
            gaia_sources, ps, obs_coords, fwhm, fov, sl, band, rot, fx, fy, sf
        )
        label = band_labels.get(band, band)
        png_b64 = _render_map(
            fcfg, image, wcs, obs_coords, label, fov, ps, sf, suptitle
        )
...
def _build_objdict(obj_props: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    return dict(
        ...
        RA_DEG=ra,
        DEC_DEG=dec,
        EPOCH=_float_or(obj_props, ["EPOCH"])
        or Time(2000.0, format="decimalyear").jd,
        PLX=_float_or(obj_props, ["Plx [mas]", "PLX"]) or 0.0,
        PMRA=_float_or(obj_props, ["PMRA [mas/yr]", "PMRA"]) or 0.0,
        PMDE=_float_or(
            obj_props, ["PMDE [mas/yr]", "PMDE", "PMDEC [mas/yr]", "PMDEC"]
        )
        or 0.0,
    )
...
def _get_gaia_sources(
    coords: SkyCoord, obstime: Time, radius: Any, max_pm: Any
) -> Dict[str, np.ndarray]:
    ...
    gaia_query = GAIA_QUERY.format(
        ra=coords.ra.deg, dec=coords.dec.deg, radius=radius.to(uu.deg).value
    )
...
def _seed_image(
    ...
    npixel_x = int(
        fov_scaled[0].to(uu.deg) // (pixel_scale * uu.pixel).to(uu.deg)
    )
    npixel_y = int(
        fov_scaled[1].to(uu.deg) // (pixel_scale * uu.pixel).to(uu.deg)
    )
    fwhm_pix = fwhm.to(uu.arcsec) / (pixel_scale * uu.pixel).to(uu.arcsec)

    wcs = _setup_wcs(
        (npixel_y, npixel_x), obs_coords, pixel_scale, rotation, flip_x, flip_y
    )
    image = np.random.normal(size=(npixel_y, npixel_x), scale=1.0, loc=0)
    nsig_psf = np.array(10 ** ((sigma_limit - gaia_sources[band]) / 2.5))

    x_sources, y_sources = wcs.all_world2pix(
        gaia_sources["ra"], gaia_sources["dec"], 0
    )
    y, x = np.mgrid[0:npixel_y, 0:npixel_x]
    ew = fwhm_pix.value / (2 * np.sqrt(2 * np.log(2)))
    cos_rot = (
        np.cos(rotation).value
        if hasattr(np.cos(rotation), "value")
        else np.cos(rotation)
    )
    sin_rot = (
        np.sin(rotation).value
        if hasattr(np.sin(rotation), "value")
        else np.sin(rotation)
    )
    ...
    in_bounds = (
        (x_sources > -margin)
        & (x_sources < npixel_x + margin)
        & (y_sources > -margin)
        & (y_sources < npixel_y + margin)
...
def _render_map(
    ...
    ra_comp_e = (
        ra_comp + (length_world[0].to(uu.deg).value * compass_frac) / cos_dec
    )
    ...
    arrow_kw = dict(
        ...
        va="center",
        color=color,
        arrowprops=dict(arrowstyle="<-", color=color, shrinkA=0.0, shrinkB=0.0),
    )
    ...
    npixel_x = int(
        field_of_view[0].to(uu.deg) // (pixel_scale * uu.pixel).to(uu.deg)
    )
    npixel_y = int(
        field_of_view[1].to(uu.deg) // (pixel_scale * uu.pixel).to(uu.deg)
    )
...
if __name__ == "__main__":
    ...
    preset_dir = (
        pathlib.Path(__file__).resolve().parents[1] / "admin" / "instruments"
    )
