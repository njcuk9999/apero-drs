...
def _build_2d_spectrum_plot(
    ...
    try:
        ...
        with _fits.open(str(filepath)) as hdul:
            fiber = _detect_fiber(
                hdul, (kw_fiber, 'AB', 'A', 'B', 'C', '')
            )
            if fiber is None:
                return _no_plot(
                    'Cannot detect fiber in FITS extensions.'
                )
            wave = np.array(
                hdul[f'Wave{fiber}'].data, dtype=float
            ).ravel()
            flux = np.array(
                hdul[f'Flux{fiber}'].data, dtype=float
            ).ravel()
            blaze = np.array(
                hdul[f'Blaze{fiber}'].data, dtype=float
            ).ravel()
    ...
    fig.line(list(wave), list(norm_flux),
             line_color='black', line_width=0.8, alpha=0.85,
...
def _build_s1d_spectrum_plot(
    ...
    try:
        ...
        tcorr = (np.array(tbl[tcorr_col], dtype=float)
                 if tcorr_col else None)
    except Exception as exc:
    ...
    fig.line(list(wave_ds), list(flux_ds),
             line_color='black', line_width=0.8, alpha=0.85,
             legend_label='Extracted 1D')
    if tcorr is not None:
        _, tcorr_ds = _downsample(wave, tcorr)
        fig.line(list(wave_ds), list(tcorr_ds),
                 line_color='red', line_width=0.8, alpha=0.75,
...
def _build_polar_plot(
    ...
    try:
        ...
        with _fits.open(str(filepath)) as hdul:
            ...
            pol = np.array(
                hdul['Pol'].data, dtype=float
            ).ravel()
            stokesi = np.array(
                hdul['StokesI'].data, dtype=float
            ).ravel()
            null1 = np.array(
                hdul['Null1'].data, dtype=float
            ).ravel()
            null2 = np.array(
                hdul['Null2'].data, dtype=float
            ).ravel()
    except Exception as exc:
    ...
    for label, y_arr, color in datasets:
        ...
        fig.line(list(w_ds), list(y_ds),
                 line_color=color, line_width=0.8, alpha=0.75,
                 legend_label=label)
    if fig.legend:
        fig.legend.location = 'top_right'
        fig.legend.click_policy = 'hide'
...
def _build_ccf_plot(
    ...
    try:
        ...
        ccf_cols = sorted([
            c for c in ccf_tbl.colnames
            if re.match(r'^CCF\d+$', c)
        ])
        if not ccf_cols:
...
def _build_tellu_temp_2d_plot(
    ...
    fig.line(list(xpos), list(spectrum),
             line_color='black', line_width=0.8, alpha=0.8,
...
def _build_lbl_rdb_plot(
    ...
    try:
        ...
        if kw_output == 'LBL_RDB_FITS':
            ...
            tbl = _Table.read(str(filepath), format='ascii.rdb',
                              fast_reader=False)
        ...
        if 'RESET_RV' in tbl.colnames:
            reset_mask = np.array(
                [bool(int(v)) for v in tbl['RESET_RV']]
            )
    except Exception as exc:
    ...
    from bokeh.models import (
        ColumnDataSource, HoverTool, Range1d, Whisker,
    )
    from bokeh.plotting import figure as bk_figure
    fig = bk_figure(
    ...
    if np.any(good):
        ...
        fig.add_layout(Whisker(
            source=src_g, base='x', upper='upper', lower='lower',
            dimension='height', line_color='black', line_alpha=0.5,
            level='underlay',
        ))
        fig.scatter('x', 'y', source=src_g, color='black',
                    marker='circle', size=5, alpha=0.7,
                    legend_label='RV')
    # -------------------------------------------------------------------------
    ...
    if np.any(reset_s):
        ...
        fig.add_layout(Whisker(
            source=src_r, base='x', upper='upper', lower='lower',
            dimension='height', line_color='orange', line_alpha=0.5,
            level='underlay',
        ))
        fig.scatter('x', 'y', source=src_r, color='orange',
                    marker='circle', size=5, alpha=0.7,
...
def _lbl_trumpet_panel(
    ...
    if np.any(bad_mask):
        ...
        fig.add_layout(Whisker(
            source=src_b, base='x', upper='upper', lower='lower',
            dimension='height',
            line_color=off_color, line_alpha=0.1, level='underlay',
        ))
        fig.scatter('x', 'y', source=src_b, color=off_color,
                    marker='circle', size=4, alpha=0.08,
    ...
    if np.any(good_mask):
        ...
        fig.add_layout(Whisker(
            source=src_g, base='x', upper='upper', lower='lower',
            dimension='height',
            line_color=on_color, line_alpha=0.4, level='underlay',
        ))
        fig.scatter('x', 'y', source=src_g, color=on_color,
                    marker='circle', size=5, alpha=0.5,
                    legend_label='σ < 300 m/s')
    # -------------------------------------------------------------------------
    # median line
    from bokeh.models import Span
    fig.add_layout(Span(
        location=median_val, dimension='width',
...
def build_filename_plot_json(
    ...
    if kw_output not in PLOTABLE_OUTPUT_TYPES:
        return _no_plot(
            f'No plot defined for output type: {kw_output}'
        )
