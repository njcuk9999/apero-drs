...
Generates matplotlib-based debug plots from htable header data and
returns them as base64-encoded PNGs.
...
- Calibration time deltas (EXT_CDT* − EXT_MJDMID vs time)
- Telluric correction map (from SC1D FITS files)

Based on apero-drs ``apero.tools.module.ari.ari_plot``.
...
def generate_debug_plots(
    ...
    """
    if not htable_rows:
        return dict(success=False, plots={},
                    error='No htable data available.')
    plots: Dict[str, Any] = {}
    for key in ('extsmax', 'effron', 'version', 'cdt'):
        try:
            ...
            image = None
            plots[key] = _plot_entry(key, image=None,
    ...
    if ftable_tcorr_rows and paths and preset:
        try:
            image = _build_tcorr_map(htable_rows, ftable_tcorr_rows,
                                     paths, preset, objname)
        except Exception:
            image = None
...
# Private helpers
# =============================================================================
def _plot_entry(key: str, image: Optional[str] = None,
                error: str = '') -> dict:
...
def _extract_column(htable_rows: List[Dict[str, Any]], key: str,
...
# =============================================================================
def _build_debug_plot(plot_key: str, htable_rows: List[Dict[str, Any]],
...
def _build_extsmax_plot(htable_rows: List[Dict[str, Any]],
                        objname: str) -> Optional[str]:
...
def _build_effron_plot(htable_rows: List[Dict[str, Any]],
                       objname: str) -> Optional[str]:
...
def _build_simple_mjd_plot(
    ...
    fig.suptitle(title)
    fig.subplots_adjust(hspace=0, left=0.1, right=0.99,
                        bottom=0.15, top=0.9)
    return _fig_to_base64(fig)


def _build_version_plot(htable_rows: List[Dict[str, Any]],
                        objname: str) -> Optional[str]:
    ...
    for v in versions_raw:
        try:
            ...
            if len(parts) >= 3:
                version_nums.append(
                    int(parts[0]) * 10000 + int(parts[1]) * 1000
                    + int(parts[2])
                )
            else:
    ...
    fig, frames = plt.subplots(nrows=2, ncols=1, figsize=(12, 5),
                               sharex=True)
    fig.set_facecolor('white')
    for frame in frames:
    ...
    # Plot version
    frames[0].plot_date(plot_dates, version_arr, fmt='.', alpha=0.5,
                        label='Version')
    frames[0].legend(loc=0)
    frames[0].set_ylabel('Version Number')
    # Plot processing date
    frames[1].plot_date(plot_dates, pdate_arr, fmt='.', alpha=0.5,
                        label='Processing Date')
    frames[1].set_xlabel('Date')
    frames[1].set_ylabel('MJD')
    frames[1].legend(loc=0)
    fig.suptitle(f'APERO Processing Debug [{objname}]')
    fig.subplots_adjust(hspace=0.05, left=0.1, right=0.99,
                        bottom=0.12, top=0.9)
    return _fig_to_base64(fig)


def _build_cdt_plot(htable_rows: List[Dict[str, Any]],
                    objname: str) -> Optional[str]:
    ...
    fig, frames = plt.subplots(nrows=len(cdt_keys), ncols=1,
                               figsize=(12, 24), sharex=True)
    fig.set_facecolor('white')
    for it, (key, is_ref) in enumerate(cdt_keys.items()):
        ...
        frame.set_facecolor(PLOT_BACKGROUND_COLOR)
        frame.grid(which='both', color='lightgray', ls='--')
        frame.plot_date(plot_dates, variable - mjd_arr, color=color,
                        fmt='.', alpha=0.5, label=key)
...
def _build_tcorr_map(
    ...
    im0 = main_1.imshow(map2d, aspect='auto', vmin=range1[0], vmax=range1[1],
    ...
    im1 = main_2.imshow(map2d_star, aspect='auto', vmin=range2[0],
                        vmax=range2[1], interpolation='nearest',
...
def _derive_sc1d_path(tcorr_row: Dict[str, Any],
    ...
    path_key_map = {
        'raw': 'PATH_RAW', 'tmp': 'PATH_PP', 'calib': 'PATH_CALIB',
        'red': 'PATH_RED', 'tellu': 'PATH_TELLU',
        'out': 'PATH_OUT', 'lbl': 'PATH_LBL',
    }
