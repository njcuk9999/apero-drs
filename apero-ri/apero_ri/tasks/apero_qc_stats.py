...
class AperoQCStats(apero_async.AperoAsyncTask):
    ...
    def __init__(self, status="pending"):
        ...
        description = (
            "Generate the quality control statistics for the "
...
def get_calib_file(
    ...
    task_logger=None,
    stop_event=None,
) -> Tuple[Dict[str, List[Dict[str, Any]]], float, int]:
    ...
    for output in hcfg:
        ...
        for raw_entry in results:
            ...
            # push into storage
            storage[output].append(
                {
                    "abspath": str(abspath),
                    "KW_FIBER": entry.get("KW_FIBER"),
                    "KW_IDENTIFIER": entry.get("KW_IDENTIFIER"),
                }
            )
...
def read_calib_headers(
    aparams: Dict[str, Any],
    cfiles: Dict[str, List[Dict[str, Any]]],
    task_logger=None,
    stop_event=None,
    ncores: int = 1,
    ...
    if use_parallel:
        executor = _make_executor(
            mp_backend, int(ncores), mp_start_method, task_logger
        )
        ...
        if executor is not None and len(files) > 1:
            ...
            for file_entry in files:
                if isinstance(file_entry, dict):
                    abspath = Path(str(file_entry.get("abspath", "") or ""))
                    db_meta = {
                        "KW_FIBER": file_entry.get("KW_FIBER"),
                        "KW_IDENTIFIER": file_entry.get("KW_IDENTIFIER"),
                    }
                else:
                    abspath = Path(str(file_entry))
                    db_meta = {"KW_FIBER": None, "KW_IDENTIFIER": None}
                futures[
                    executor.submit(_read_one_calib_header, str(abspath), hkeys)
                ] = (abspath, db_meta)
            for fut in as_completed(futures):
                if stop_event is not None and stop_event.is_set():
                    ...
                    break
                row = fut.result()
                _, db_meta = futures.get(fut, (None, {}))
                if db_meta.get("KW_FIBER") is not None:
                    row["KW_FIBER"] = db_meta.get("KW_FIBER")
                if db_meta.get("KW_IDENTIFIER") is not None:
                    row["KW_IDENTIFIER"] = db_meta.get("KW_IDENTIFIER")
                header_dict[output].append(row)
                done_files += 1
                if callable(progress_callback):
                    try:
                        progress_callback(
            ...
            for file_entry in files:
                ...
                if stop_event is not None and stop_event.is_set():
                    ...
                    break
                row = _read_one_calib_header(str(abspath), hkeys)
                if db_meta.get("KW_FIBER") is not None:
                    row["KW_FIBER"] = db_meta.get("KW_FIBER")
                if db_meta.get("KW_IDENTIFIER") is not None:
                    row["KW_IDENTIFIER"] = db_meta.get("KW_IDENTIFIER")
...
def _read_one_calib_header(
    abspath: str, hkeys: Dict[str, Any]
) -> Dict[str, Any]:
...
def _make_executor(
    mp_backend: str, ncores: int, mp_start_method: str, task_logger=None
):
    ...
    if mp_backend == "processes":
        try:
            ...
            return ProcessPoolExecutor(
                max_workers=max(1, ncores), mp_context=ctx
            )
        except Exception as exc:
            if callable(task_logger):
                task_logger(
                    f"QC stats process pool init failed ({exc}); "
