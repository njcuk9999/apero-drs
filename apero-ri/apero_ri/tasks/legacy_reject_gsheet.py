...
Merge policy per IDENTIFIER:
- Prefer the newest row using LAST_UPDATE timestamp, falling back to
    DATE_ADDED when LAST_UPDATE is blank.
- If WHO is blank in the selected row, use WHO from Google Sheet.
- If LAST_UPDATE is blank after merge, copy DATE_ADDED.
"""
...
def _parse_iso_ts(value: Any) -> Optional[datetime]:
    """Parse an ISO-like timestamp text into a timezone-aware datetime."""
    text = str(value or '').strip()
    if text == '':
        return None
    try:
...
def _row_timestamp(row: Dict[str, Any]) -> Optional[datetime]:
    """Return comparison timestamp for one row.

    Preference order:
    1) LAST_UPDATE
    2) DATE_ADDED (fallback when LAST_UPDATE is blank/invalid)
    """
    dt = _parse_iso_ts(row.get('LAST_UPDATE'))
    if dt is not None:
        return dt
    return _parse_iso_ts(row.get('DATE_ADDED'))


def _coalesce_text(primary: Any, secondary: Any) -> str:
    """Return first non-empty stripped text from two candidates."""
    first = str(primary or '').strip()
    if first != '':
        return first
    return str(secondary or '').strip()


def _pick_newest_row(
    local_row: Dict[str, Any],
    sheet_row: Dict[str, Any],
) -> Tuple[Dict[str, Any], str]:
    """Pick baseline row by newest timestamp, preferring sheet on ties."""
    local_dt = _row_timestamp(local_row)
    sheet_dt = _row_timestamp(sheet_row)
    if local_dt is None and sheet_dt is None:
        return dict(sheet_row), 'sheet'
    if local_dt is None:
        return dict(sheet_row), 'sheet'
    if sheet_dt is None:
        return dict(local_row), 'local'
    if local_dt > sheet_dt:
        return dict(local_row), 'local'
    return dict(sheet_row), 'sheet'
...
def _merge_reject_frames(local_df: pd.DataFrame,
    ...
    for row_it, ident in enumerate(merged_ids, start=1):
        local_row = dict(local_map.get(ident, {}))
        sheet_row = dict(sheet_map.get(ident, {}))
        row, _winner = _pick_newest_row(local_row, sheet_row)
        row[mod.ID_COLUMN] = ident

        row['DATE_ADDED'] = _coalesce_text(
            row.get('DATE_ADDED'),
            _coalesce_text(local_row.get('DATE_ADDED'),
                           sheet_row.get('DATE_ADDED')),
        )
        if row['DATE_ADDED'] == '':
            row['DATE_ADDED'] = now

        # WHO rule: prefer newest row; if blank, use Google Sheet value.
        row['WHO'] = _coalesce_text(row.get('WHO'), sheet_row.get('WHO'))
        if row['WHO'] == '':
            row['WHO'] = 'googlesheet'

        # LAST_UPDATE rule: prefer newest row; if blank, use DATE_ADDED.
        row['LAST_UPDATE'] = _coalesce_text(
            row.get('LAST_UPDATE'),
            _coalesce_text(local_row.get('LAST_UPDATE'),
                           sheet_row.get('LAST_UPDATE')),
        )
        if row['LAST_UPDATE'] == '':
            row['LAST_UPDATE'] = row['DATE_ADDED']

        # Keep free-text/comment from the newest row when available.
        row['COMMENT'] = _coalesce_text(
            row.get('COMMENT'),
            _coalesce_text(local_row.get('COMMENT'),
                           sheet_row.get('COMMENT')),
        )
