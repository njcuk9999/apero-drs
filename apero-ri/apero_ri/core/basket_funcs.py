...
def _resolve_path(
    entry: Dict[str, Any], profile_cfg: Dict[str, Any]
) -> Optional[Path]:
    """
    ...
    if not isinstance(paths_cfg, dict):
        paths_cfg = {}
    base = str(
        paths_cfg.get(path_key) or profile_cfg.get(path_key) or ""
    ).strip()
    if not base:
        return None
...
def _save_job_meta(username: str, job_id: str, meta: Dict[str, Any]) -> None:
    path = _job_meta_path(username, job_id)
    path.parent.mkdir(parents=True, exist_ok=True)
    with open(path, "w", encoding="utf-8") as fh:
...
def get_job_chunk_path(
    username: str, job_id: str, chunk_idx: int
) -> Optional[Path]:
    """
        ...
        if fmt == "native" and len(file_pairs) == 1:
            ...
            for c_idx, chunk in enumerate(chunks_files):
                if actual_fmt == "tar.gz":
                    out_name = (
                        f"{base_name}_part{c_idx + 1}.tar.gz"
                        if multi
...
def _send_download_ready_email(
    to_email: str, job_id: str, chunk_metas: List[Dict[str, Any]]
) -> None:
...
def load_ftable_rows(
    ...
    try:
        ...
        generated_at = data.get("generated_at") or (
            metadata.get("GENERATED_AT") if isinstance(metadata, dict) else None
        )
...
def filter_accessible_rows(
    rows: List[Dict[str, Any]], accessible_run_ids: Set[str]
) -> List[Dict[str, Any]]:
