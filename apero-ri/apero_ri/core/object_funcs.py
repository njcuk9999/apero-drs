...
def _percentile(sorted_values: Sequence[float],
    ...
    frac = kpos - kfloor
    return float(
        sorted_values[kfloor] * (1.0 - frac)
        + sorted_values[kceil] * frac
    )
...
def _nanmedian(values: Iterable[Any]) -> Optional[float]:
    ...
    """
    vals = sorted(
        v for v in (_safe_float(x) for x in values) if v is not None
    )
...
def _min_time(rows: Iterable[Dict[str, Any]],
...
def _max_time(rows: Iterable[Dict[str, Any]],
              key: str) -> Optional[str]:
    """
...
def _qc_counts(rows: Iterable[Dict[str, Any]]) -> Dict[str, int]:
    ...
    passed = sum(
        ...
        if int(r.get('PASSED_ALL_QC') or 0) == 1
    )
    return {
        'total': total,
        'passed': passed,
...
def _load_instrument_profile(instrument_profile_file: str,
                              ) -> Dict[str, Any]:
    """
...
# =============================================================================
def _header_label(profile_data: Dict[str, Any],
                  section: str, key: str,
                  default: str) -> str:
    """
    ...
    sec = (headers.get(section, {})
           if isinstance(headers.get(section, {}), dict) else {})
    item = (sec.get(key, {})
            if isinstance(sec.get(key, {}), dict) else {})
    label = str(item.get('label', '') or '').strip()
    return label or default


def _header_key(profile_data: Dict[str, Any],
                section: str, key: str,
                default: str) -> str:
    """
...
def _resolve_lbl_path(path_lbl: str, obs_dir: str,
...
def _compute_lbl_flavor_stats(rdb_path: Path,
    ...
    vrad = [v for v in (_safe_float(x) for x in vrad_raw)
            if v is not None]
    svrad = [v for v in (_safe_float(x) for x in svrad_raw)
             if v is not None]
    rjd = [v for v in (_safe_float(x) for x in rjd_raw)
           if v is not None]
    reset_flags = [
        bool(int(x) if x not in (None, '', 'None') else 0)
        for x in reset_raw
    ]
    ...
    if p10 is not None and p90 is not None:
        ...
        stats['valid_velocity_domain'] = (
            f'{ylim_lo:.2f} to {ylim_hi:.2f} m/s'
        )
        stats['spurious_low_points'] = sum(
            1 for v in vrad if v < ylim_lo
        )
        stats['spurious_high_points'] = sum(
            1 for v in vrad if v > ylim_hi
        )
    else:
...
def build_object_page_stats(*,
    ...
    fkind_map = {
        'raw': 'raw', 'pp': 'pp', 'ext': 'ext',
        'tcorr': 'tcorr', 'ccf': 'ccf', 'lbl': 'lbl', 'lbl_rdb': 'lbl_rdb',
    }
    ...
    htable_path = (
        objects_dir
        / f'htable_{obj_row.get("OBJNAME", objname)}.json'
    )
    ...
    target_info = {
        ...
        'ra_source': _first_present(
            obj_row, ['RA source', 'RA_SOURCE']
        ),
        'dec_deg': _first_present(obj_row, ['Dec [Deg]', 'DEC_DEG']),
        'dec_source': _first_present(
            obj_row, ['Dec source', 'DEC_SOURCE']
        ),
        'finder_chart': None,
        'teff_k': _first_present(obj_row, ['Teff [K]', 'TEFF']),
        'teff_source': _first_present(
            obj_row, ['Teff source', 'TEFF_SOURCE']
        ),
        'spectral_type': _first_present(
            obj_row, ['SpT', 'SPECTRAL_TYPE']
        ),
        ...
        'radial_velocity': _first_present(
            obj_row, ['RV [km/s]', 'RV']
        ),
        'radial_velocity_source': _first_present(
            obj_row, ['RV source', 'RV_SOURCE']
        ),
        'aliases': _first_present(
            obj_row, ['ALIASES', 'ALIASES_STR']
        ),
    ...
    spectrum_info = {
        'dprtypes': _first_present(
            obj_row, ['DPRTYPE', 'ALL_DPRTYPES']
        ),
        'raw_total': _fmt_count(
            raw_stats['total'], raw_stats_all['total']
        ),
        ...
        'pp_total': _fmt_count(
            pp_stats['total'], pp_stats_all['total']
        ),
        'pp_passed': _fmt_count(
            pp_stats['passed'], pp_stats_all['passed']
        ),
        'pp_failed': _fmt_count(
            pp_stats['failed'], pp_stats_all['failed']
        ),
        ...
        'ext_total': _fmt_count(
            ext_stats['total'], ext_stats_all['total']
        ),
        'ext_passed': _fmt_count(
            ext_stats['passed'], ext_stats_all['passed']
        ),
        'ext_failed': _fmt_count(
            ext_stats['failed'], ext_stats_all['failed']
        ),
        ...
        'tcorr_last_processed': _max_time(
            tcorr_rows, 'LAST_MODIFIED'
        ),
    ...
    lbl_vers_hdrkey = _header_key(
        preset, 'lbl', 'LBL_VERSION', 'LBL_VERS'
    )
    ...
    for fname, row in sorted(lbl_rdb_by_file.items()):
        ...
        fstats = (
            _compute_lbl_flavor_stats(
                rdb_full, fits_full, lbl_vers_hdrkey
            )
        ...
        lbl_flavors.append({
            ...
            'rv_abs_dev_percentiles': fstats.get(
                'rv_abs_dev_percentiles'
            ),
            'spurious_low_points': fstats.get('spurious_low_points'),
            'spurious_high_points': fstats.get(
                'spurious_high_points'
            ),
            'valid_velocity_domain': fstats.get(
    ...
    if lbl_flavors:
        ...
        summary_n_nights = len(
            {str(r.get('OBS_DIR')) for r in lbl_rows
             if r.get('OBS_DIR')}
        )
    ...
    lbl_info = {
        ...
        'spurious_low_points': (
            lbl_flavors[0].get('spurious_low_points')
            if lbl_flavors else None
        ),
        'spurious_high_points': (
            lbl_flavors[0].get('spurious_high_points')
            if lbl_flavors else None
        ),
        'n_reset_rv_points': (
            lbl_flavors[0].get('n_reset_rv_points')
            if lbl_flavors else None
        ),
        'systemic_velocity': (
            lbl_flavors[0].get('systemic_velocity')
            if lbl_flavors else None
        ),
        # raw float systemic velocity in m/s for downstream use
        'vsys_ms': (
            lbl_flavors[0].get('systemic_velocity_ms')
            if lbl_flavors else None
        ),
        'valid_velocity_domain': (
            lbl_flavors[0].get('valid_velocity_domain')
            if lbl_flavors else None
        ),
        'flavors': lbl_flavors,
    ...
    ccf_info = {
        'mask_used': _join_unique(
            r.get('CCF_MASK') for r in htable_rows_acc
        ),
        ...
        'total_files': _fmt_count(
            ccf_stats['total'], ccf_stats_all['total']
        ),
        'passed_qc': _fmt_count(
            ccf_stats['passed'], ccf_stats_all['passed']
        ),
        'failed_qc': _fmt_count(
            ccf_stats['failed'], ccf_stats_all['failed']
        ),
    ...
    for row in ext_rows_all:
        ...
        if not obs_dir:
            continue
        entry = nights.setdefault(obs_dir, {
            'obs_dir': obs_dir,
            'ext_rows_all': [],
            'ext_rows': [],
            'tcorr_rows_all': [],
    ...
    for nkey in sorted(nights.keys()):
        ...
        time_series.append({
            ...
            'dprtypes': (
                ', '.join(dprtypes) if dprtypes else None
            ),
            'ext_files_label': (
...
# =============================================================================
def load_object_htable_rows(objects_dir: Path,
                            objname: str) -> List[Dict[str, Any]]:
    """
...
def load_object_preset(instrument_profile_file: str,
                       ) -> Dict[str, Any]:
    """
...
def load_object_table_row(objects_dir: Path,
                          objname: str) -> Dict[str, Any]:
    """
...
def load_object_ftable_rows(objects_dir: Path,
                            objname: str,
                            fkind: str) -> List[Dict[str, Any]]:
    """
    ...
    return _load_json_rows(
        objects_dir / f'ftable_{fkind}_{objname}.json'
    )
...
def _percentile(sorted_values: Sequence[float], pct: float) -> Optional[float]:
    ...
    if kfloor == kceil:
        return float(sorted_values[kfloor])
    frac = kpos - kfloor
    return float(sorted_values[kfloor] * (1.0 - frac)
                 + sorted_values[kceil] * frac)
...
def _resolve_lbl_path(path_lbl: str, obs_dir: str, filename: str) -> Optional[Path]:
    """Resolve an LBL file path with basic path-traversal prevention."""
    if not path_lbl or not filename:
        return None
    base = Path(path_lbl).resolve()
...
def _compute_lbl_flavor_stats(rdb_path: Path,
                              fits_path: Optional[Path],
                              lbl_version_hdrkey: str) -> Dict[str, Any]:
    """Read an LBL_RDB .rdb file and its optional FITS companion and return stats.
    ...
    reset_flags = [bool(int(x) if x not in (None, '', 'None') else 0)
                   for x in reset_raw]
    ...
    if fits_path is not None:
        try:
            hdr = _fits.getheader(str(fits_path))
            hdrkey = lbl_version_hdrkey or 'LBL_VERS'
            stats['lbl_version'] = str(hdr.get(hdrkey, '') or '').strip() or None
        except Exception:
            stats['lbl_version'] = None
    else:
        stats['lbl_version'] = None
...
def build_object_page_stats(*,
                            ...
                            instrument_profile_file: str = '',
                            path_lbl: str = '') -> Dict[str, Any]:
    """Build object-page sections and labels from object/ftable/htable sources."""
    profile_dir = base_dir / 'tasks' / instrument / profile_id
    objects_dir = profile_dir / 'objects'
    ...
    labels = {
        ...
        'spectrum': {
            'pp_version': _header_label(preset, 'pp', 'PP_VERSION', 'Version [pp]'),
            'ext_version': _header_label(preset, 'ext', 'EXT_VERSION', 'Version [ext]'),
        },
        'ccf': {
            'ccf_version': _header_label(preset, 'ccf', 'CCF_VERSION', 'Version [ccf]'),
        },
        'time_series': {
            'snr_order_15': _header_label(preset, 'ext', 'EXT_Y', 'SNR[Order 15]'),
            'snr_order_60': _header_label(preset, 'ext', 'EXT_H', 'SNR[Order 60]'),
        },
    }
    ...
    target_info = {
        ...
        'spectral_type_source': _first_present(obj_row, ['SpT source', 'SPT_SOURCE']),
        'pmra': _first_present(obj_row, ['PMRA [mas/yr]', 'PMRA']),
        'pmdec': _first_present(obj_row, ['PMDE [mas/yr]', 'PMDEC [mas/yr]', 'PMDEC']),
        'parallax': _first_present(obj_row, ['Plx [mas]', 'PLX']),
        'radial_velocity': _first_present(obj_row, ['RV [km/s]', 'RV']),
        'radial_velocity_source': _first_present(obj_row, ['RV source', 'RV_SOURCE']),
        ...
        'ob_names_in_headers': _join_unique(r.get('PP_VERSION') for r in htable_rows_acc),
        'pi_names_in_headers': _join_unique(r.get('PP_PI_NAME') for r in htable_rows_acc),
        'project_run_names_in_headers': _join_unique(r.get('PP_PROG_ID') for r in htable_rows_acc),
    }
    ...
    spectrum_info = {
        ...
        'pp_last_mid': _max_time(pp_rows, 'MID_OBS_TIME'),
        'pp_last_processed': _max_time(pp_rows, 'LAST_MODIFIED'),
        'pp_version': _join_unique(r.get('PP_VERSION') for r in htable_rows_acc),
        ...
        'ext_last_mid': _max_time(ext_rows, 'MID_OBS_TIME'),
        'ext_last_processed': _max_time(ext_rows, 'LAST_MODIFIED'),
        'ext_version': _join_unique(r.get('EXT_VERSION') for r in htable_rows_acc),
        ...
        'tcorr_version': _join_unique(r.get('TCORR_VERSION') for r in htable_rows_acc),
        'median_snr_y': _format_number(_nanmedian(r.get('EXT_Y') for r in htable_rows_acc), ndp=2),
        'median_snr_h': _format_number(_nanmedian(r.get('EXT_H') for r in htable_rows_acc), ndp=2),
    }
    ...
    for fname, row in sorted(lbl_rdb_by_file.items()):
        ...
        lbl_flavors.append({
            ...
            'systemic_velocity': fstats.get('systemic_velocity'),
            'systemic_velocity_ms': fstats.get('systemic_velocity_ms'),
            'rv_uncertainty_percentiles': fstats.get('rv_uncertainty_percentiles'),
    ...
    lbl_info = {
        # Summary (backwards-compatible flat fields; populated from first flavor)
        'measurement_count': summary_measurement_count,
        'n_nights': summary_n_nights,
        'lbl_version': summary_lbl_version,
        'rv_uncertainty_percentiles': lbl_flavors[0].get('rv_uncertainty_percentiles') if lbl_flavors else None,
        'rv_abs_dev_percentiles': lbl_flavors[0].get('rv_abs_dev_percentiles') if lbl_flavors else None,
        'spurious_low_points': lbl_flavors[0].get('spurious_low_points') if lbl_flavors else None,
        'spurious_high_points': lbl_flavors[0].get('spurious_high_points') if lbl_flavors else None,
        'n_reset_rv_points': lbl_flavors[0].get('n_reset_rv_points') if lbl_flavors else None,
        'systemic_velocity': lbl_flavors[0].get('systemic_velocity') if lbl_flavors else None,
        # Raw float systemic velocity in m/s for downstream use (e.g. BERV plot)
        'vsys_ms': lbl_flavors[0].get('systemic_velocity_ms') if lbl_flavors else None,
        'valid_velocity_domain': lbl_flavors[0].get('valid_velocity_domain') if lbl_flavors else None,
        # Per-flavor sub-sections
        'flavors': lbl_flavors,
        'total_rdb_files': _fmt_count(len(lbl_rdb_rows_acc), len(lbl_rdb_rows_all)),
    }

    ccf_info = {
        'mask_used': _join_unique(r.get('CCF_MASK') for r in htable_rows_acc),
        'systemic_velocity': _format_number(_nanmedian(r.get('CCF_DV') for r in htable_rows_acc), ndp=3),
        'fwhm': _format_number(_nanmedian(r.get('CCF_FWHM') for r in htable_rows_acc), ndp=3),
        ...
        'last_mid': _max_time(ccf_rows, 'MID_OBS_TIME'),
        'last_processed': _max_time(ccf_rows, 'LAST_MODIFIED'),
        'ccf_version': _join_unique(r.get('CCF_VERSION') for r in htable_rows_acc),
    }
    ...
    for nkey in sorted(nights.keys()):
        ...
        time_series.append({
            ...
            'num_ext': _fmt_count(len(ext_n), len(ext_n_all)),
            'num_tcorr': _fmt_count(len(tc_n), len(tc_n_all)),
            'seeing': _format_number(_mean(r.get('EXT_SEEING') for r in n_hrows), ndp=2),
            'airmass': _format_number(_mean(r.get('EXT_AIRMASS') for r in n_hrows), ndp=3),
            'mean_exptime': _format_number(_mean(r.get('EXT_EXPTIME') for r in n_hrows), ndp=3),
            'total_exptime': _format_number(_sum(r.get('EXT_EXPTIME') for r in n_hrows), ndp=3),
            'snr_order_15': _format_number(_mean(r.get('EXT_Y') for r in n_hrows), ndp=2),
            'snr_order_60': _format_number(_mean(r.get('EXT_H') for r in n_hrows), ndp=2),
...
def load_object_htable_rows(objects_dir: Path, objname: str) -> List[Dict[str, Any]]:
    """Load and return htable rows for *objname* from *objects_dir*.
...
def load_object_ftable_rows(objects_dir: Path, objname: str,
                            fkind: str) -> List[Dict[str, Any]]:
    """Load and return ftable rows for *objname* and file-kind *fkind*.
