...
def save_mount_log(
    mount_name: str, log_lines: List[str], source: str = "mount"
) -> None:
...
def _resolve_connection_target(mount_config: Dict[str, Any]) -> Dict[str, str]:
    ...
    if mode == "ssh_config_host":
        ...
        return {
            ...
            "sshfs_target": (
                ...
                if ssh_config_host and remote_path
                else ""
            ),
    ...
    return {
        ...
        "sshfs_target": (
            f"{ssh_target}:{remote_path}" if ssh_target and remote_path else ""
        ),
        "display_target": ssh_target,
    }


def _build_sshfs_mount_command(
    mount: Dict[str, Any], key_path: Path, local_mount: Path
) -> List[str]:
...
def _throttle_ssh_target(
    target: str, action: str, min_interval_s: float = SSH_CONNECT_MIN_INTERVAL_S
) -> str:
...
def add_ssh_key(key_name: str, key_content: str) -> Dict[str, Any]:
    ...
    if not re.match(r"^[a-zA-Z0-9_-]+$", key_name):
        return {
            "ok": False,
            "error": "Key name must contain only alphanumeric"
    ...
    try:
        ...
        return {
            "ok": True,
            "message": f'SSH key "{key_name}" added successfully.',
        }
    except Exception as e:
...
def delete_ssh_key(key_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "ok": True,
            "message": f'SSH key "{key_name}" deleted successfully.',
        }
    except Exception as e:
...
def add_mount(mount_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if existing:
            return {
                "ok": False,
                "error": f'Mount "{mount_config["name"]}" already exists.',
            }
        ...
        if result["ok"]:
            return {
                "ok": True, "message": f'Mount "{
                    mount_config["name"]}" added successfully.', }
        return result
    except Exception as e:
    ...
    try:
        ...
        if mount.get("status") == "mounted":
            ...
            if not unmount_result["ok"]:
                return {
                    "ok": False,
                    "error": f'Cannot delete mounted volume. Unmount first: {
                        unmount_result["error"]}',
                }

        cfg["mounts"] = [
            m for m in cfg["mounts"] if m.get("name") != mount_name
        ]
        result = save_sshfs_config(cfg)
        ...
        if result["ok"]:
            return {
                "ok": True,
                "message": f'Mount "{mount_name}" deleted successfully.',
            }
        return result
    except Exception as e:
...
def update_mount(
    original_name: str, mount_config: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    try:
        ...
        index = next(
            (i for i, m in enumerate(mounts) if m.get("name") == original_name),
            None,
        )
        if index is None:
        ...
        for i, mount in enumerate(mounts):
            if i != index and mount.get("name") == new_name:
                return {
                    "ok": False,
                    "error": f'Mount "{new_name}" already exists.',
                }
        ...
        if result["ok"]:
            return {
                "ok": True,
                "message": f'Mount "{new_name}" updated successfully.',
            }
        return result
    except Exception as e:
...
def _prepare_mount_dir(local_mount: Path, log: List[str]) -> None:
    ...
    try:
        ...
        log.append(
            f"Mount directory exists but inaccessible (stale FUSE?) — "
        ...
        log.append(
            f"fusermount exit={fuse.returncode}  "
...
def mount_sshfs(mount_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not mount:
            return {
                "ok": False,
                "error": f'Mount "{mount_name}" not found.',
                "log": log,
            }
        ...
        if mount.get("status") == "mounted":
            ...
            log.append(
                f"Config says mounted but mountpoint check failed — "
                f"resetting status"
            )
        ...
        if not key_path.exists():
            return {
                "ok": False,
                "error": f'SSH key "{ssh_key}" not found.',
                "log": log,
            }
        ...
        save_mount_log(mount_name, log, source="mount")
        return {"ok": False, "error": "Mount operation timed out.", "log": log}
    except Exception as e:
        log.append(f"Exception: {e}")
        save_mount_log(mount_name, log, source="mount")
        return {"ok": False, "error": f"Failed to mount: {str(e)}", "log": log}
...
def unmount_sshfs(mount_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if mount.get("status") != "mounted":
            return {
                "ok": True,
                "message": f'Mount "{mount_name}" is not mounted.',
            }
        ...
        result = subprocess.run(
            ["umount", local_mount], capture_output=True, text=True, timeout=10
        )
        ...
        return {
            "ok": True,
            "message": f'Mount "{mount_name}" unmounted successfully.',
        }
    except subprocess.TimeoutExpired:
        ...
        for cmd in attempts:
            ...
            if result.returncode == 0:
                ...
                return {
                    "ok": True,
                    "message": (
                        f'Mount "{mount_name}" lazy-unmounted '
                        "successfully."
                    ),
                    "command": " ".join(cmd),
                }
...
# -------------------------------------------------------------------------
def _check_mount_status_worker(mount: Dict[str, Any], out_conn) -> None:
...
def test_ssh_connection(
    ...
    lines = [
        line.strip()
        for line in (login.stdout or "").splitlines()
        if line.strip()
    ]
    ...
    return {
        "ok": True,
        "message": (
            "SSH authentication succeeded and remote path "
            f'"{remote_path}" is accessible.'
        ),
...
def health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        if not status_data["ok"]:
            return {
                "status": "error",
                "message": f'Failed to check mount status: {
                    status_data["error"]}',
            }
        ...
        if failed_count > 0:
            return {
                "status": "error", "message": f"{failed_count} of {
                    len(all_mounts)} SSHFS mounts have connection issues.", }

        if mounted_count == len(all_mounts):
            return {
                "status": "ok",
                "message": f"All {len(all_mounts)} SSHFS mounts are mounted.",
            }

        unmounted = len(all_mounts) - mounted_count
        return {
            "status": "warning",
            "message": f"{unmounted} of {
                len(all_mounts)} SSHFS mounts are not currently mounted.",
        }
    except Exception as e:
        return {
            "status": "error",
