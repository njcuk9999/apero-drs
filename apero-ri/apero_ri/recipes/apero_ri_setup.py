...
def _get_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Local data directory to initialize "
            "(default: ~/.ari or bootstrap config)"
        ),
    )
    parser.add_argument(
        "--host",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Run setup even if setup_state.yaml already marks "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Dry-run mode: run the full setup wizard "
...
def main():
    ...
    local_data_dir = (
        Path(args.data_dir).expanduser() if args.data_dir else default_dir
    )

    if test_mode:
        print("[TEST MODE] No files will be written to disk.")
        print(
            f"[TEST MODE] Would create directory layout under {local_data_dir}"
        )
    ...
    if is_setup_complete(local_data_dir) and not args.force and not test_mode:
        ...
        print(
            "Run `apero_ri_run` to start the application, "
    ...
    if not args.data_dir:
        print(
            "Tip: you can change this path on the first setup "
            "page in the browser."
        )
