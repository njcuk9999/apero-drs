#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Engineering check: warning cryo2 state."""
...
CHECK_NAME = 'ENG_WARN_CRYO2'
CHECK_HUMAN_NAME = 'ENG: Warning Cryo2 State'
CHECK_TYPE = 'raw'
...
def _description(cfg: dict) -> str:
    logic = (
        'np.all(np.char.strip(' + str(cfg.get('key1', '')) + ') == '
        + repr(str(cfg.get('val1', ''))) + ')'
    )
...
def check_function(instrument: str, obs_dir: str,
    ...
    """Run warning-cryo2 state validation for one observation directory.
    ...
    values = np.char.strip(np.array(table[key1]).astype(str))
    use = masks[key1]
    x = values[use]
    if len(x) == 0:
