...
def _description(cfg: dict) -> str:
    logic = (
        'np.nanstd(' + str(cfg.get('key1', '')) + ' - '
        + str(cfg.get('key2', '')) + ') < ' + str(cfg.get('val1', ''))
    )
...
def check_function(instrument: str, obs_dir: str,
    ...
    """Run FP setpoint-RMS validation for one observation directory.
    ...
    defs[key2] = dict(key=key2, dtype='float')
