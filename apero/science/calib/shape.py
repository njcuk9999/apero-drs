...
def calculate_dxmap(params, recipe, fpdata, lprops, fiber, **kwargs):
    ...
    # the max widths of each order
    order_widths = np.zeros(nbo)
    ...
    for order_num in range(nbo):
        ...
        for f_it, _fiber in enumerate(list(acc.keys())):
            tmp_ypix[f_it] = mp.val_cheby(acc[_fiber][order_num], xpix,
                                              domain=[0, dim2])
    ...
    for banana_num in range(nbanana):
        ...
        for order_num in range(nbo):
            ...
            coeffs = mp.nanpolyfit(xsection[keep], dxsection[keep],
                                   fit_slope_deg)
            # log slope at center
            s_xpix = dim2 // 2
