...
# -*- coding: utf-8 -*-
"""
Pseudo constants (function) definitions for SPIP
...
# Define variables
# =============================================================================
__NAME__ = 'config.instruments.spip.pseudo_const'
__INSTRUMENT__ = 'SPIP'
...
class PseudoConstants(pseudo_const.DefaultPseudoConstants):
    ...
    def FILEMOD(self) -> base_class.ImportModule:
        ...
        if self.filemod is not None:
            return self.filemod
        # set module name
        module_name = 'apero.core.instruments.spip.file_definitions'
        # try to import module
        try:
            self.filemod = base_class.ImportModule('spip.file_definitions',
                                                   module_name)
            return self.filemod
    ...
    def RECIPEMOD(self) -> base_class.ImportModule:
        ...
        # set module name
        module_name = 'apero.core.instruments.spip.recipe_definitions'
        # try to import module
        try:
            strmod = 'spip.recipe_definitions'
    ...
    def HEADER_FIXES(self, params: ParamDict, header: Any,
                     ...
                     objdbm: Any = None) -> Any:
        """
        For SPIP the following keys may or may not be present (older data
    ...
    def GET_LSD_ORDER_RANGES(self) -> List[List[float]]:
        """
        Define the valid wavelength ranges for each order in SPIP.
    ...
    def FILEINDEX_HEADER_COLS(self) -> DatabaseColumns:
        ...
        header_cols.add(name='KW_IDENTIFIER', datatype='VARCHAR(80)',
                        is_index=True)
        header_cols.add(name='KW_IRELDATE', datatype='VARCHAR(80)')
    ...
    def FIBER_DOUBLETS(self, fiber: str) -> bool:
        """
        whether we have orders coming in doublets (i.e. SPIP AB --> A + B)
    ...
    def FIBER_LOC_TYPES(self, fiber: str) -> str:
        ...
        another fiber for localisation e.g. SPIP A or B --> AB
    ...
    def FIBER_LOC_COEFF_EXT(self, coeffs: np.ndarray,
        ...
        for SPIP we have either AB,A,B of size 98 orders or C of size 49
    ...
    def GET_EPOCH(self, params, header) -> float:
        ...
        """
        # SPIP stores the equinox in decimal years
    ...
    def COMBINE_FILE_SUFFIX(self, basenames: List[str], suffix: str):
        ...
        for basename in basenames:
            ...
            # for SPIP we have a an odocode followed by a letter as the prefix
...
def get_drs_mode(params: ParamDict, header: Any, hdict: Any) -> Tuple[Any, Any]:
    """
    Assign the drs mode to the drs (for SPIP based on the polar mode)
...
def construct_dprtype(params: ParamDict, filename: str,
    ...
    """
    from apero.core.instruments.spip import file_definitions
