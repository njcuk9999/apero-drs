...
KDict.set(key='TRG_TYPE', comment='target or sky object',
                   group='ppraw')
...
KDict.set(key='NEXP', comment='Total number of exposures within the sequence',
            combine_method='1', group='raw')

# define the pi name HEADER key
KDict.set(key='PI_NAME', comment='The PI of the program', group='raw')

# define the run id HEADER key
KDict.set(key='RUNID', comment='queue run id', group='raw')
...
KDict.set(key='RA_DEG', unit=uu.deg, comment='Target right ascension',
             group='raw')

# define the observation dec HEADER key
KDict.set(key='DEC_DEG', unit=uu.deg, comment='Target declination',
              group='raw')
...
# define the observation name
KDict.set(key='OBJNAME', comment='Target name (alternate)',
                   group='raw')

# define the observation equinox HEADER key
KDict.set(key='OBJEQUIN', datatype='decimalyear',
...
KDict.set(key='AIRMASS', comment='Airmass at start of observation',
               group='raw')
...
KDict.set(key='SBRHB1_P', group='raw')

# define the second polar sequence key
KDict.set(key='SBRHB2_P', group='raw')
...
KDict.set(key='DRSMODE', comment='APERO-mode (Spectroscopy or Polar)',
                group='ppraw')
...
KDict.set(key='THRM_RAT', comment='Ratio 1 used for thermal correction')

# the ratio method used for thermal correction
KDict.set(key='THRM_RU',
