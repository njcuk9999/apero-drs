...
def get_database(p, max_time=None, update=False):
    ...
    :param p: parameter dictionary, ParamDict containing constants
        Must contain at least:
                ...
                log_opt: string, log option, normally the program name
    :param max_time: str, maximum time allowed for all telluDB entries
                     format = (YYYY-MM-DD HH:MM:SS.MS)
    :param update: bool, if False looks for "telluDB' in p, and if found does
                   not load new database

    :return c_database: dictionary, the telluDB database in form:
                    c_database[key] = [dirname, filename]
        lines in telluDB must be in form:
    ...
    for l_it, line in enumerate(lines):
        ...
        # will crash if we don't have 5 variables --> thus log and exit
            ...
            lock.close()
            os.remove(lock_file)
            emsg1 = 'Incorrectly formatted line in telluDB - function = {0}'
        ...
        telludb_fmt = spirouConfig.Constants.DATE_FMT_TELLUDB()
        # get unix time (assume t_fmt is in GMT/UTC)
        t_fmt_unix = spirouMath.stringtime2unixtime(t_fmt, telludb_fmt)
        # get human time in UTC/GMT
        t_human = spirouMath.unixtime2stringtime(t, telludb_fmt)
        if t_fmt_unix != t:
            lock.close()
            os.remove(lock_file)
            emsg1 = 'Times do not match in telluDB'
    ...
    if len(keys) == 0:
        ...
        emsg1 = 'There are no entries in telluDB'
        emsg2 = '   Please check telluDB file at {0}'.format(telludb_file)
        emsg3 = '   function = {0}'.format(func_name)
        WLOG('error', p['LOG_OPT'], [emsg1, emsg2, emsg3])
    # Finally we only want to keep one teluDB file for each key
    #     This depends on 'tellu_db_match'
    #     If tellu_db_match = 'older' - select the newest file that is older
    #                                   than max_time
    #     If tellu_db_match = 'closest' - select the file that is closest to
    #                                     max_time (newer OR older)
    try:
        t_database = choose_keys(p, utimes, keys, dirnames, filenames)
...
def choose_keys(p, utimes, keys, dirnames, filenames):
    """
    Choose the keys in the telluric database based on 'tellu_db_match'
    if 'tellu_db_match' == older, then choose the most recent of each key that
    is closest but older than the time in p['MAX_TIME_UNIX']
    if 'tellu_db_match' == closest, then choose the most recent of each key
    ...
    :param p: parameter dictionary, ParamDict containing constants
            Must contain at least:
                max_time_unix: float, the unix time to use as the time of
                               reference
                TELLU_DB_MATCH: string, either "closest" or "older"
    ...
    :param keys: list of strings, the keys in telluDB
    ...
    :return c_database: dict, the telluric database, with keys equal to the
    ...
    # get match
    match = p['TELLU_DB_MATCH']
    # log telluDB match method
    wmsg = 'TelluDB loaded with method "{0}"'
    ...
    for ukey in ukeys:
        ...
        # if we have none then cause error
            ...
            telludb_file = spirouConfig.Constants.TELLUDB_MASTERFILE(p)
            emsg1 = ('There are no entries for key="{0}" in telluDB file with '
...
def copy_files(p, header=None):
    """
    Copy the files from telluDB to the reduced folder
       p['DRS_DATA_REDUC']/p['ARG_NIGHT_NAME']
    based on the latest telluDB files from header, if there is not header file
    use the parameter dictionary "p" to open the header in 'arg_file_names[0]'
        ...
        if os.path.exists(newloc):
            ...
            # if it isn't then copy over it
                ...
                try:
                    ...
                    wmsg = 'Tellu. file: {0} copied in dir {1}'
                    WLOG('', p['LOG_OPT'], wmsg.format(filename, reduced_dir))
                except IOError:
                    emsg1 = ('I/O problem on input file from telluDB: {0}'
                             '').format(oldloc)
...
def get_file_name(p, key, hdr=None, filename=None, required=True):
    """
    Get the filename for "key" in the telluric database (for use when
    the telluric database is not needed for more than one use and does
    not exist already (i.e. called via spirouDB.GetTelluDatabase() )
    ...
    :param p: parameter dictionary, ParamDict containing constants
        Must contain at least:
                ...
                reduced_dir: string, the reduced data directory
                             (i.e. p['DRS_DATA_REDUC']/p['ARG_NIGHT_NAME'])
    :param key: string, the key to look for in the telluric database
    ...
    :param filename: string or None, if defined this is the filename returned
                     (means telluric database is not used)
    :param required: bool, if True code generates log exit else raises a
                     ConfigError (to be caught)

    :return read_file: string, the filename in telluric database for
                       "key" (selected via unix_time in telluDB)
    """
    ...
    if 'telluDB' not in p:
        ...
        # get telluDB
        t_database, p = get_database(p, acqtime)
    else:
        t_database = p['TELLUDB']

    # Check that "KEY" is in tellu database and assign value if it is
    if filename is not None:
        read_file = filename
    else:
        if key in t_database:
            rawfilename = t_database[key][1]
        else:
            emsg1 = ('Telluric database has no valid "{0}" entry '
...
def get_check_lock_file(p):
    ...
    if os.path.exists(lock_file):
        WLOG('warning', p['LOG_OPT'], 'TelluDB locked. Waiting...')
...
def write_files_to_master(p, lines, keys, lock, lock_file):
    ...
    try:
        ...
        f.writelines(lines)
        f.close()
        wmsg = 'Updating Tellu Data Base with {0}'
...
def read_master_file(p, lock, lock_file):
    ...
    try:
        ...
        os.remove(lock_file)
        # log and exit
        emsg1 = 'TelluDB master file: {0} can not be found!'.format(masterfile)
        emsg2 = '   function = {0}'.format(func_name)
        WLOG('error', p['LOG_OPT'], [emsg1, emsg2])
    else:
        # write database line entry to file
