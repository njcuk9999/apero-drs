...
for iord in range(0,n_ord):
	...
	for ite_banana in range(3):
		...
		while current_max>(.3*maxpeak):
			# find the peak
			id = np.nanargmax(profil)
			current_max=profil[id]
			xpospeak[ii]=id
			...
			if (ybot[id]>1):
				...
				box = data2[ybot[id]:ytop[id],xpix[id]-9:xpix[id]+9]
				...
				for i in range(wpix):
					spline=InterpolatedUnivariateSpline(xx,box1[i,:],ext=1,k=1)
				...
				# perform a collapse along the y axis and take the peak value of this profile
				...
				with warnings.catch_warnings(record=True) as _:
					fit=np.polyfit(slopes[pixmax-1:pixmax+2],medmax[pixmax-1:pixmax+2],2)
				...
				if (ii % 10)==0:
					print('p ratio: ',current_max/maxpeak,ii,allslopes[ii],' iord = ',iord)
				...
				if ii<300:
					...
					if (maxpix>5):    # TODO: Why 5?
						all_slices[:,:,ii % 10] += box2[:,maxpix-3:maxpix+3]
				ii+=1


			# we NaN the peak so that the code can move to the
			# next peak
			profil[xpix[id]-2:xpix[id]+2] = np.nan   # TODO: Why +/- 2?

		# keep all valid slopes and peak positions
		g=np.isfinite(allslopes)
		# no slope can be a >10 sigma outlier
		g &= ( np.abs(allslopes-np.nanmedian(allslopes))<10*np.nanmedian(np.abs(allslopes-np.nanmedian(allslopes))) )
		xpospeak=xpospeak[g]
		...
		slope=np.polyval(np.polyfit(xpospeak,allslopes,2),np.array(range(4088)))
		...
		plt.title('robust sig dx='+str(np.nanmedian(np.abs(dx-np.nanmedian(dx)))))
...
pyfits.writeto( (slope_file.split('_'))[0]+'_dxmap.fits' ,master_dxmap,clobber=True)
