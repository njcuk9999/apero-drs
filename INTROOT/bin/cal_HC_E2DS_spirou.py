...
def main(night_name=None, files=None):
    ...
    for fiber in p['FIB_TYP']:
        ...
        for x_it in range(len(loc['X_CUT_POINTS_2'])):
            ...
            if sig_littrow > p['QC_RMS_LITTROW_MAX']:
                fmsg = ('Littrow test (x={0}) failed (sig littrow = '
                        '{1:.3f} > {2:.3f})')
                fargs = [x_cut_point, sig_littrow, p['QC_RMS_LITTROW_MAX']]
                fail_msg.append(fmsg.format(*fargs))
            # check if min/max littrow is out of bounds
            if np.max([max_littrow, min_littrow]) > p['QC_DEV_LITTROW_MAX']:
                fmsg = ('Littrow test (x={0}) failed (min|max dev = '
                        '{1:.3f}|{2:3f} > {1:3f})')
                fargs = [x_cut_point, min_littrow, max_littrow,
                         p['QC_DEV_LITTROW_MAX']]
                fail_msg.append(fmsg.format(*fargs))
        ...
        spirouImage.WriteImage(p['FITSFILENAME'], data, hdict)
        ...
        e2dscopy_fitsfilename = spirouConfig.Constants.WAVE_E2DS_COPY(p)
        # make a copy of the E2DS file for the calibBD
        spirouImage.WriteImage(e2dscopy_fitsfilename, data, hdict)
        ...
        loc['FLAT'] = np.ones(data.shape)
        # set blaze to all ones
        loc['BLAZE'] = np.ones(data.shape)
        # set sources
