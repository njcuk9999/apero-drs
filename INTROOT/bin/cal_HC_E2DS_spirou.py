...
def main(night_name=None, files=None):
    ...
    p = spirouImage.GetAcqTime(p, loc['HCHDR'], name='acqtime', kind='unix')
    bjdref = p['ACQTIME']
...
def part1b(p, loc):
    ...
    WLOG('', p['LOG_OPT'] + p['FIBER'], wmsg.format(p['FIBER']))
    # fit lines
    ll = loc['LITTROW_EXTRAP_SOL_1'][p['IC_HC_N_ORD_START_2']:p['IC_HC_N_ORD_FINAL_2']]
...
#    spirouImage.WriteImage(p['FITSFILENAME'], loc['HCDATA'], hdict)
    ...
    if p['QC']:
        ...
        # update the master calib DB file with new key
        spirouCDB.UpdateMaster(p, keydb, os.path.split(e2dscopy_filename)[-1], loc['HCHDR'])
