...
def load_ari_params(params: ParamDict) -> ParamDict:
    ...
    # ARI calib page directory
    ari_calib_page = os.path.join(ari_dir, 'calib_page')
    # ARI default working
    ...
    params.set('TOOLS.ARI.DIR', value=ari_dir, source=func_name)
    params.set('TOOLS.ARI.OBJ_YAMLS', value=ari_obj_yamls, source=func_name)
    params.set('TOOLS.ARI.RECIPE_YAMLS', value=ari_recipe_yamls, source=func_name)
    params.set('TOOLS.ARI.OBJ_PAGES', value=ari_obj_pages, source=func_name)
    params.set('TOOLS.ARI.RECIPE_PAGES', value=ari_recipe_pages, source=func_name)
    params.set('TOOLS.ARI.CALIB_PAGE', value=ari_calib_page, source=func_name)
    params.set('TOOLS.ARI.DWORKING_DIR', value=dworking, source=func_name)
    # add a list for ARI_EXTRA (html files not generated by sphinx)
    params.set('TOOLS.ARI.EXTRAS', value=[], source=func_name)
    # ----------------------------------------------------------------------
    # make sure these directories exist
    for path in [ari_dir, ari_obj_yamls, ari_recipe_yamls, ari_obj_pages,
                 ari_calib_page, ari_recipe_pages]:
...
def make_calib_page(params: ParamDict) -> TableFile:
    ...
    :return: TableFile - the table file to add to the profile page
    """
    # ------------------------------------------------------------------
    # create a table file instance
