...
__NAME__ = 'apero.instruments.spirou.constants.py'
...
CDict.set('AREL_RDELTA', value=0.0, source=__NAME__, group=cgroup)

# Define the time delta (in years) to add to the KW_IRELDATE key by default
#   for APERO files
#   if there is no other way to determine the public release date (can be zero)
CDict.set('AREL_ADELTA', value=0.0, source=__NAME__, group=cgroup)

# Define the time delta (in years) to add to the KW_IRELDATE key by default
#   for LBL files
#   if there is no other way to determine the public release date (can be zero)
CDict.set('AREL_LDELTA', value=1.0, source=__NAME__, group=cgroup)
...
CDict.set('FIRST_ORDER_JUMP', value=dict(AB=2, A=0, B=0, C=1, group=cgroup),
          source=__NAME__, group=cgroup)

# Maximum number of order to use
CDict.set('MAX_NUM_ORDERS', value=dict(AB=98, A=49, B=49, C=49, group=cgroup),
          source=__NAME__, group=cgroup)

# Number of fibers
CDict.set('SET_NUM_FIBERS', value=dict(AB=2, A=1, B=1, C=1, group=cgroup),
          source=__NAME__, group=cgroup)

# Get the science and reference fiber to use in the CCF process
CDict.set('CCF', value=['AB', 'C'], source=__NAME__, group=cgroup)

# List the individual fiber names
CDict.set('INDIVIDUAL', value=['A', 'B', 'C'], source=__NAME__, group=cgroup)

# List the sky fibers to use for the science channel and the calib channel
CDict.set('SKYFIBERS', value=['AB', None], source=__NAME__, group=cgroup)
...
CDict.set('LED_FLAT_FILE', value='None', source=__NAME__, group=cgroup)
...
CDict.set('CX_AMP_FLUX', value=1.490711e-04, source=__NAME__, 
          author='EA', group=cgroup)

# Define amplitude of the flux-dependent along-readout-axis 1st
# derivative component
# TODO: Add calculation to static apero-utils.general.apero_statics.corr_xtalk.py
CDict.set('CX_AMP_DFLUX', value=8.080468e-04, source=__NAME__, 
          author='EA', group=cgroup)

# Define amplitude of the flux-dependent along-readout-axis 2nd
# derivative component
# TODO: Add calculation to static apero-utils.general.apero_statics.corr_xtalk.py
CDict.set('CX_AMP_D2FLUX', value=-6.523985e-04, source=__NAME__, 
          author='EA', group=cgroup)
...
CDict.set('REF_MIN_EXPTIME', value=10, source=__NAME__, group=cgroup)
...
CDict.set('FULL_FLAT', value='detector_flat_full.fits', source=__NAME__, group=cgroup)

# Percentile to normalise to when normalising and median filtering
# image [percentage]
CDict.set('NORM_PERCENTILE', value=90.0, source=__NAME__, group=cgroup)
...
CDict.set('KER_AMP', value=47, source=__NAME__, group=cgroup)
...
CDict.set('YDET_MIN', value=65, source=__NAME__, author='EA', group=cgroup)

# Define the maximum detector position where the centers of the orders should
# fall (across order direction)
CDict.set('YDET_MAX', value=3050, source=__NAME__, author='EA', group=cgroup)
...
CDict.set('MIDDLE_PIXEL_COL', value=2044, source=__NAME__, author='EA', group=cgroup)

# Half spacing between orders
CDict.set('HALF_ORDER_SPACING', value=45, source=__NAME__, group=cgroup)
...
CDict.set('COL_SEP_FITTING', value=20, source=__NAME__, group=cgroup)

# Definition of the extraction window size (half size)
CDict.set('EXT_WIN_SIZE', value=15, source=__NAME__, group=cgroup)
...
CDict.set('ORDER_WID_MIN', value=10, source=__NAME__, group=cgroup)
...
CDict.set('MAX_RMS_WID', value=1.0, source=__NAME__, group=cgroup)

# Maximum fractional peak-to-peak for sigma-clip order fit (width)
CDict.set('MAX_PTP_WID', value=10.0, source=__NAME__, group=cgroup)
...
CDict.set('COEFF_SIGCLIP', value=7, source=__NAME__, group=cgroup)

# Defines the fit degree to fit in the coefficient cleaning
CDict.set('COEFFSIG_DEG', value=7, source=__NAME__, group=cgroup)

# Define the maximum value allowed in the localisation (cuts reddest orders)
CDict.set('MAX_YPIX_VAL', value=4088, source=__NAME__, group=cgroup)
...
CDict.set('FP_REF_PERCENT_THRES', value=90.0, source=__NAME__, group=cgroup)

# Define the largest standard deviation allowed for the shift in
# x or y when doing the shape reference fp linear transform
CDict.set('QC_LTRANS_RTHRES', value=0.1, source=__NAME__, group=cgroup)
...
CDict.set('REF_VALIDFP_PTILE', value=80, source=__NAME__, group=cgroup)
...
CDict.set('BLAZE_SINC_MED_SIZE', value=25, source=__NAME__, group=cgroup)
...
CDict.set('QC_MAX_RMS', value=0.05, source=__NAME__, group=cgroup)
...
CDict.set('S1D_WAVEEND', value=2500, source=__NAME__, group=cgroup)
...
CDict.set('S1D_BIN_UVEL', value=1.0, source=__NAME__, group=cgroup)
...
CDict.set('S1D_PLOT_ZOOM1',
          value=[990, 1245, 1570, 2000, 2400], source=__NAME__, group=cgroup)

# Define the wavelength upper bounds for s1d plots
# (must be a string list of floats) defines the upper wavelength in nm
CDict.set('S1D_PLOT_ZOOM2',
          value=[1050, 1285, 1670, 2100, 2500], source=__NAME__, group=cgroup)
...
CDict.set('THERMAL_CORR', value=True, source=__NAME__, group=cgroup)
...
CDict.set('CORR_TYPE1', value=['OBJ', 'POLAR'], source=__NAME__, group=cgroup)

# Define DPRTYPEs we need to correct thermal background using
# method 2
CDict.set('CORR_TYPE2',
          value=['FP', 'HC', 'HCONE', 'HCTWO', 'FLAT'], source=__NAME__, group=cgroup)

# Define the order to perform the thermal background scaling on
CDict.set('SCALE_ORDER', value=48, source=__NAME__, group=cgroup)
...
CDict.set('MIN_WAVE_LIM', value=1900, source=__NAME__,
          author=base.AUTHORS['EA'], group=cgroup)

# width of the median filter used for the background
CDict.set('FILT_WID', value=101, source=__NAME__, group=cgroup)

# Define thermal red limit (in nm)
CDict.set('RED_WAVE_LIM', value=2500, source=__NAME__, group=cgroup)

# Define thermal blue limit (in nm)
CDict.set('BLUE_WAVE_LIM', value=2450, source=__NAME__, group=cgroup)

# maximum tapas transmission to be considered completely opaque for the
# purpose of background determination in order 49.
CDict.set('THRES_TAPAS', value=0.010, source=__NAME__, group=cgroup)

# Define the percentile to measure the background for correction type 2
CDict.set('ENVELOPE_PTILE', value=10, source=__NAME__, group=cgroup)

# Define the order to plot on the thermal debug plot
CDict.set('PLOT_START_ORDER', value=35, source=__NAME__, group=cgroup)

# Define the dprtypes for which to apply the excess emissivity file
CDict.set('EXCESS_DPRTYPES', value=['DARK_DARK_TEL'], source=__NAME__, group=cgroup)

# Define the thermal emissivity file
CDict.set('EXCESS_EMISSIVITY_FILE', value='excess_emissivity.csv',
          source=__NAME__, group=cgroup)
...
CDict.set('CAVITY_FIT_DEG', value=9, source=__NAME__, author='EA', group=cgroup)
...
# absolute numbering
CDict.set('MIN_FP_LINES', value=30, source=__NAME__, author='EA', group=cgroup)
...
CDict.set('FIBER_COMP_PLOT_ORD', value=35, source=__NAME__, group=cgroup)

# Define the line list file (located in the DRS_WAVE_DATA directory)
CDict.set('LINELIST_FILE', value='catalogue_UNe.csv',
...
CDict.set('TORDER_START', value=79, source=__NAME__, author='EA', group=cgroup)
...
CDict.set('NORD_FINAL', value=47, source=__NAME__, author='EA', group=cgroup)
...
CDict.set('NSIG_MIN_HC', value=15, source=__NAME__, author='EA', group=cgroup)
...
CDict.set('REF_FP_NLOW', value=9000, source=__NAME__, author='EA', group=cgroup)
...
CDict.set('REF_FP_POLYINV', value=10, source=__NAME__, author='EA', group=cgroup)
...
CDict.set('ORDER_BINS', value=3, source=__NAME__, author='EA', group=cgroup)

# Define the number of bins in spatial direction to use in the resolution map
CDict.set('SPATIAL_BINS', value=3, source=__NAME__, author='EA', group=cgroup)
...
CDict.set('FITBOX_EWMIN', value=0.7, source=__NAME__,
          author='EA', group=cgroup)  # 0.7
CDict.set('FITBOX_EWMAX', value=1.1, source=__NAME__,
...
CDict.set('ORDER_INIT2', value=1, source=__NAME__, group=cgroup)

# Define the order to end the Littrow fit at for the HC wave solution
CDict.set('ORDER_FINAL1', value=47, source=__NAME__, group=cgroup)

# Define the order to end the Littrow fit at for the FP wave solution
# TODO: Note currently used
CDict.set('ORDER_FINAL2', value=47, source=__NAME__, group=cgroup)
...
CDict.set('DOPD0', value=24498000, source=__NAME__, group=cgroup)
...
CDict.set('P2P_WID_CUT', value=15, source=__NAME__, group=cgroup)
...
CDict.set('DPRLIST', value='OBJ_FP, POLAR_FP', source=__NAME__,
          author='EA', group=cgroup)
...
CDict.set('REJECT_DRS_HKEY', value='KW_IDENTIFIER', source=__NAME__, group=cgroup)
...
CDict.set('RLIST_GSHEET_MAINLIST_ID', value='1398289367',
          source=__NAME__, group=cgroup)

# Define the google sheet name to use for the reject list
CDict.set('RLIST_GSHEET_SHEETNAME', value='SPIROU', source=__NAME__, group=cgroup)
...
CDict.set('MODEL_EXT_SNR_ORDER', value=35, source=__NAME__, author='EA', group=cgroup)
...
CDict.set('ALLOWED_DPRTYPES', value=[], source=__NAME__, group=cgroup)
...
CDict.set('FIBER_TYPE', value='AB', source=__NAME__, group=cgroup)
...
CDict.set('CLEAN_OHLINES', value=True, source=__NAME__, group=cgroup)
...
# background)
CDict.set('REMOVE_ORDERS', value=[45, 46, 47, 48], source=__NAME__, group=cgroup)
...
CDict.set('ABSO_EXPO_KWID', value=4.95, source=__NAME__, group=cgroup)
...
CDict.set('DWATER_ABSO', value=4.0, source=__NAME__, group=cgroup)
...
CDict.set('WATER_BOUNDS', value=[0.1, 10], source=__NAME__, group=cgroup)

# set the plot order for the finite resolution plot (somewhere around 1.45 um)
CDict.set('FINITE_RES_ORDER', value=26, source=__NAME__, author='EA', group=cgroup)
...
# to the telluDB
CDict.set('QC_SNR_ORDER', value=33, source=__NAME__, group=cgroup)
...
CDict.set('QC_AIRMASS_DIFF', value=0.3, source=__NAME__, group=cgroup)
...
CDict.set('SNR_ORDER', value=33, source=__NAME__, group=cgroup)
...
CDict.set('HOTSTAR_KER_VEL', value=100, source=__NAME__, group=cgroup)
...
CDict.set('TEFF_MASK_TABLE', value='teff_masks.csv', source=__NAME__,
          datatype='csv', group=cgroup)
...
CDict.set('DEFAULT_STEP', value=0.5, source=__NAME__, group=cgroup)
...
CDict.set('N_ORD_MAX', value=48, source=__NAME__, group=cgroup)
...
CDict.set('BERV_CORRECT', value=True, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Whether or not to correct for SOURCE RV shift before calculate polarimetry
CDict.set('SOURCE_RV_CORRECT', value=False, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Define the polarimetry method
CDict.set('METHOD', value='Ratio', source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Whether or not to interpolate flux values to correct for wavelength shifts between exposures
CDict.set('INTERPOLATE_FLUX', value=True, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Select stokes I continuum detection algorithm
CDict.set('STOKESI_CONT_DET_ALG', value='IRAF', source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Select polar continuum detection algorithm
CDict.set('CONT_DEL_ALG', value='IRAF', source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Normalize Stokes I (True or False)
CDict.set('NORM_STOKES_I', value=True, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Remove continuum polarization
CDict.set('REMOVE_CONT', value=True, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Apply polarimetric sigma-clip cleaning (Works better if continuum is removed)
CDict.set('CLEAN_BY_SIGCLIP', value=True, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Define number of sigmas within which apply clipping
CDict.set('NSIGCLIP', value=4, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Define the reddest wavelength to use throughout polar code
CDict.set('REDDEST_THRES', value=2450, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)
...
# set value
CDict.set('TELLU_BANDS', value=bands, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)
...
# 'spline3'
CDict.set('CONT_FITFUNC_STOKESI', value='spline3', source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# function to fit to the polar continuum: must be 'polynomial' or 'spline3'
CDict.set('CONT_FITFUNC_POLAR', value='spline3', source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# stokes i continuum fit function order: 'polynomial': degree or 'spline3':
# number of knots
CDict.set('CONT_FUNC_ORDER_STOKESI', value=60, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# polar continuum fit function order: 'polynomial': degree or 'spline3':
# number of knots
CDict.set('CONT_FUNC_ORDER_POLAR', value=20, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)
...
CDict.set('LSD_DIR', value='lsd', source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)
...
# - filekey = 't*_g'
CDict.set('LSD_FILE_KEY', value='marcs_t*g50_all', source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Define minimum lande of lines to be used in the LSD analyis
CDict.set('LSD_MIN_LANDE', value=0.0, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Define maximum lande of lines to be used in the LSD analyis
CDict.set('LSD_MAX_LANDE', value=10.0, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# If mask lines are in air-wavelength then they will have to be
# converted from air to vacuum
CDict.set('LSD_CCFLINES_AIRWAVE', value=False, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Define minimum line depth to be used in the LSD analyis
CDict.set('LSD_MIN_LINEDEPTH', value=0.03, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Define maximum line depth to be used in the LSD analyis
CDict.set('LSD_MAX_LINEDEPTH', value=1, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Define initial velocity (km/s) for output LSD profile
CDict.set('V0', value=-150.0, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Define final velocity (km/s) for output LSD profile
CDict.set('VF', value=150.0, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Define number of points for output LSD profile
CDict.set('NP', value=151, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Renormalize data before LSD analysis
CDict.set('NORMALIZE', value=False, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Remove edges of LSD profile
CDict.set('REMOVE_EDGES', value=True, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)

# Define the guess at the resolving power for lsd profile fit
CDict.set('RES_POWER_GUESS', value=50000.0, source=__NAME__,
          author=base.AUTHORS['EM'], group=cgroup)
...
CDict.set('POLAR_FIT_CONT', value=True, source=__NAME__, group=cgroup)

# turn on the polar continuum debug plot
CDict.set('POLAR_CONTINUUM', value=True, source=__NAME__, group=cgroup)

# turn on the polar results debug plot
CDict.set('POLAR_RESULTS', value=True, source=__NAME__, group=cgroup)

# turn on the polar stokes i debug plot
CDict.set('POLAR_STOKES_I', value=True, source=__NAME__, group=cgroup)
...
CDict.set('OBJECT_TYPES', value=['OBJECT'], source=__NAME__, group=cgroup)
...
CDict.set('OBJ_SCI_SQL', value=' AND KW_OBSTYPE="OBJECT"',
          source=__NAME__, group=cgroup)
