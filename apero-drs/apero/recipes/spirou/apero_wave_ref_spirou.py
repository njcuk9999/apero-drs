...
# -*- coding: utf-8 -*-
"""
apero_wave_ref_spirou.py [obs dir] [HC_HC files] [FP_FP files]

APERO wavelength solution reference calibration recipe for SPIROU
...
# Define variables
# =============================================================================
__NAME__ = 'apero_wave_ref_spirou.py'
...
def main(obs_dir: Optional[str] = None, hcfiles: Optional[List[str]] = None,
    ...
    """
    Main function for apero_wave_ref
...
def __main__(recipe: DrsRecipe, params: ParamDict) -> Dict[str, Any]:
    ...
    for it in range(num_files):
        ...
        # Ref night A, B and C (not AB) -> we force the coefficients to
        # be the ones found with AB, fit_cavity = False,
        # fit_full_cavity_pol = False
        fit_cavity = False
        fit_full_cavity_pol = False
        ...
        for fiber in fiber_types:
            ...
            wavefile = wave.write_wavesol(params, *fpargs, ref=True)
            # -----------------------------------------------------------------
            # Write cavity file (for reference fiber)
            # -----------------------------------------------------------------
            if fiber == ref_fiber:
                # cavity args
            ...
            wmargs = [hc_e2ds_file, fp_e2ds_file, wavefile, hclines,
                      fplines, fiber]
            out = wave.write_wave_lines(params, recipe, *wmargs, ref=True)
            ...
            # ----------------------------------------------------------
            if passed and params['INPUTS']['DATABASE']:
                # only save cavity file for reference fiber
                if fiber == ref_fiber:
