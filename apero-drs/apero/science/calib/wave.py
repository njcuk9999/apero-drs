...
def get_waveref_filename(params: ParamDict, recipe: DrsRecipe, fiber: str,
    ...
    Order of preference: WAVESOL_REF >> WAVESOL_DEFAULT
    ...
    keys = ['WAVESOL_REF', 'WAVESOL_DEFAULT']
    filename, out_wave = None, None
    # loop around possible wave reference keys
    for k_it in range(len(keys)):
        # first check for wave reference generated by the drs
        ...
        if filename is not None:
            break
    # ------------------------------------------------------------------------
    # if we still have no filename we have to use the static file
    if filename is None:
        filename = get_static_wavesol(params, fiber)
    # ------------------------------------------------------------------------
...
def get_static_wavesol(params: ParamDict, fiber: str) -> str:
    # first check for wave reference generated by the drs
    gkwargs = dict(block_kind='asset', required=False)
    out_wave_file = drs_file.get_file_definition(params, 'STATIC_WAVE_REF',
                                                 **gkwargs)
    out_wave = out_wave_file.newcopy(params=params, fiber=fiber)
    ...
    out_wave.construct_filename(path=cal_path, fiber=fiber)
    # push filename to variable
    filename = out_wave.filename
    # return filename
    return filename
...
def get_wave_solution_from_wavefile(params: ParamDict, recipe: DrsRecipe,
    ...
    :param ref: bool, if True we require the reference file (overrides the
                   wave filename
...
def get_wavesolution(params: ParamDict, recipe: DrsRecipe,
    ...
    3. From calibDB if force=True or reference=True or keys missing from
       infile/header
    ...
    #    exception is when we are getting a reference
    # ------------------------------------------------------------------------
...
def get_wavelines(params: ParamDict, recipe, fiber: str,
    ...
    out_wave_fp = drs_file.get_file_definition(params, 'WAVE_FPLIST_REF',
                                               block_kind='red')
    out_wave_hc = drs_file.get_file_definition(params, 'WAVE_HCLIST_REF',
                                               block_kind='red')
    # get calibration key
...
def calc_wave_lines(params: ParamDict, recipe: DrsRecipe,
    ...
    func_name = display_func('calc_wave_lines', __NAME__)
    # -------------------------------------------------------------------------
...
def calc_wave_sol(params: ParamDict, recipe: DrsRecipe,
    ...
    #  1. This is a reference night AB, we do not want an achromatic fit and all
    #      coefficients should be fitted
    #  2. This is a non-reference night AB, we must have an input (the reference
    ...
    if fit_full_cavity_pol and fit_cavity:
        ...
        if cavity_update is not None:
            #  Scenario 2. This is a non-reference night AB, we must have an
            #              input (the reference night solution) cavity and only
...
def process_fibers(params: ParamDict, recipe: DrsRecipe,
    ...
    other fibers are calculated similarly to reference fiber but with
        fit_cavity = False
        fit_achromatic = False

    :param params: ParamDict, parameter dictionary of constants
    :param recipe: DrsRecipe instance, the recipe calling this function
    :param refprops: ParamDict, the wave properties of the reference file
    ...
    # mhcl = refprops['HCLINES']
    # mfpl = refprops['FPLINES']
    ...
    for fiber in fiber_types:
        ...
        # ---------------------------------------------------------------------
        # skip all this if this is the reference fiber (its all been done)
        if fiber == ref_fiber:
            solutions[fiber] = refprops
            continue
        # ---------------------------------------------------------------------
        # deal with having to shift the reference wave sol (for large difference
        #    between fibers)
        offset = fiber_offset[fiber]
        scale = fiber_scale[fiber]
        refprops1 = wprop_pixel_wave_shift(refprops, offset=offset, scale=scale)
        # ---------------------------------------------------------------------
        # generate the hc reference lines
        hcargs = dict(e2dsfile=hc_e2ds_file, wavemap=refprops1['WAVEMAP'],
                      iteration='1 fiber {0}'.format(fiber))
        hclines = calc_wave_lines(params, recipe, **hcargs)
        # generate the fp reference lines
        fpargs = dict(e2dsfile=fp_e2ds_file, wavemap=refprops1['WAVEMAP'],
    ...
    # ----------------------------------------------------------------------
    # plot comparison between reference fiber and fibers
    # ----------------------------------------------------------------------
    recipe.plot('WAVE_FIBER_COMPARISON', solutions=solutions, ref=refprops,
                order=None, reffiber=ref_fiber)
    recipe.plot('WAVE_FIBER_COMP', solutions=solutions, ref=refprops,
                order=plot_order, reffiber=ref_fiber)
    recipe.plot('SUM_WAVE_FIBER_COMP', solutions=solutions, ref=refprops,
                order=plot_order, reffiber=ref_fiber)
    # ----------------------------------------------------------------------
...
def wave_meas_diff(params: ParamDict, ref_fiber: str,
...
def wave_quality_control(params: ParamDict, solutions: Dict[str, ParamDict],
    ...
    # --------------------------------------------------------------
    for fiber in fiber_types:
        # do not compare reference to reference
        if fiber == ref_fiber:
            continue
        ...
        qc_names.append('DV[{0} - {1}]'.format(ref_fiber, fiber))
        qargs = [ref_fiber, fiber, rv_thres]
    ...
    # --------------------------------------------------------------
    # get reference RV [km/s] --> [m/s]
    ref_rvs = rvprops[ref_fiber]['CCF_FIT_COEFFS'][:, 1] * 1000
    ...
    for fiber in fiber_types:
        ...
        qc_names.append('CCFRV[{0}] - CCFRV[{1}]'.format(ref_fiber, fiber))
        qargs = [ref_fiber, fiber, rv_thres]
        qc_logic.append('abs(CCFRV[{0}] - CCFRV[{1}]) > {2} m/s'.format(*qargs))
        # print to screen
        pargs = [ref_fiber, fiber, meddiff]
...
def write_wavesol(params: ParamDict, recipe: DrsRecipe, fiber: str,
                  ...
                  qc_params: List[list], ref: bool = False) -> DrsFitsFile:
    """
    ...
    :param ref: bool, if True we write a reference wave solution
...
def write_wave_lines(params: ParamDict, recipe: DrsRecipe,
                     ...
                     fplines: Table, fiber: str, ref: bool = False,
                     # TODO: REMOVE LATER
                     file_kind=None):
    """
    ...
    :param ref: bool, if this is a reference recipe
    :param file_kind: str temp file naming override (remove later)

    :return: None - writes to file
    """
    # set function name
    # _ = display_func('write_wave_lines', __NAME__)
    # ------------------------------------------------------------------
    # write hc lines
    # ------------------------------------------------------------------
    # get copy of instance of wave file (WAVE_HCMAP)
...
def write_cavity_file(params: ParamDict, recipe: DrsRecipe,
    ...
    # ------------------------------------------------------------------
    # get copy of instance of wave file (WAVE_HCMAP)
    cavfile = recipe.outputs['WAVEREF_CAVITY'].newcopy(params=params)
    # construct the filename from file instance
