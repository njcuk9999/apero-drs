...
def tellu_preclean(params, recipe, infile, wprops, fiber, rawfiles, combine,
    ...
    # ----------------------------------------------------------------------
    # get pconst
    pconst = load_functions.load_pconfig(instruments=select.INSTRUMENTS)
    # ----------------------------------------------------------------------
    ...
    # ----------------------------------------------------------------------
    # return props and the updated template
...
def clean_ohline_pca(params, recipe, image, wavemap, **kwargs):
    ...
    for ncomp in range(n_components):
        ...
        ohpcshift = wave_core.wave_to_wave(ohspec, ohwave, wavemap)
...
def qc_exit_tellu_preclean(params, recipe, image, image_e2ds_ini, infile,
    ...
    props.set_sources(keys, func_name)
    # ----------------------------------------------------------------------
    # return props and the updated template props
    return props, template_props
...
def shift_template(params: ParamDict, recipe: DrsRecipe,
    ...
    # shift template e2ds
    template_e2ds = wave_core.wave_to_wave(tprops['TEMP_S2D'],
                                          wavemap_ref / dvshift,
    ...
    # shift template s1d
    template_s1d = wave_core.wave_to_wave(tmp_s1d, tmp_wave / dvshift,
                                         tmp_wave, reshape=False)
    template_s1d_deconv = wave_core.wave_to_wave(tmp_s1d_deconv,
                                                tmp_wave / dvshift,
                                                tmp_wave, reshape=False)
