...
def polarimetry_diff_method(params: ParamDict, props: ParamDict,
    ...
    # ---------------------------------------------------------------------
    try:
        outs = polar_core.polarimetry_diff_method(data, errdata, nexp)
    except ValueError as e:
        raise AperoCodedException(params, message=str(e),
                                  targs=[func_name])
    pol_arr, pol_err_arr, null1_arr, null2_arr = outs
    # -------------------------------------------------------------------------
...
def polarimetry_ratio_method(params: ParamDict, props: ParamDict,
    ...
    # ---------------------------------------------------------------------
    try:
        outs = polar_core.polarimetry_ratio_method(data, errdata, nexp)
    except ValueError as e:
        raise AperoCodedException(params, message=str(e),
                                  targs=[func_name])
    pol_arr, pol_err_arr, null1_arr, null2_arr = outs
...
def calculate_stokes_i(params: ParamDict, props: ParamDict,
    ...
    # delegate numerical work to the profile-independent core module
    # ---------------------------------------------------------------------
    stokesi_arr, stokesierr_arr = polar_core.calculate_stokes_i(data, errdata,
                                                                nexp)
...
def remove_continuum_polarization(params: ParamDict, props: ParamDict
    ...
    reddest_thres = params['OBJ.POL.GEN.REDDEST_THRES']
    # ---------------------------------------------------------------------
    # delegate numerical work to the profile-independent core module
    # ---------------------------------------------------------------------
    pol, order_cont_pol = polar_core.remove_continuum_polarization(
        wldata, pol, stokesi, wavemap, cont_pol, reddest_thres)
    # -------------------------------------------------------------------------
    # update POL
    props['POL'] = pol
...
def normalize_stokes_i(params: ParamDict, props: ParamDict) -> ParamDict:
    ...
    # delegate numerical work to the profile-independent core module
    # ---------------------------------------------------------------------
    outs = polar_core.normalize_stokes_i(wldata, pol, stokesi, stokesierr,
                                        wavemap, cont_flux, reddest_thres)
    stokesi, stokesierr, order_cont_flux = outs
    # -------------------------------------------------------------------------
