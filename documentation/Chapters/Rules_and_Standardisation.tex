...
As in previous version of the DRS the printing and logging is controlled by a function. In this version of the DRS this is in \definevariable{\spirouLog.logger} but in most recipes/modules this is aliased to \definevariable{WLOG}. The \definevariable{WLOG} function controls both the printing to the screen (standard output) and to a log file. Where and how this is done is controlled by several variables.
...
\noindent The `char' and level are a dictionary pair in the form `level = char' and is controlled by \definevariable{trig\_key} (see section \ref{ch:variables:log_print}) i.e. by default the level char pairs are:
\begin{pythonbox}
dict(all=' ', error='!', warning='@', info='*', graph='~')
\end{pythonbox}

\noindent The level also determines whether or not a message is shown in the screen (standard output) or in the log. A log message will be shown if it has a numeric value (defined in \definevariable{write\_level}) higher than that set in \definevariable{PRINT\_LEVEL} for printing to the screen (standard output) or set in \definevariable{LOG\_LEVEL} for printing to the log.
...
\noindent In addition to logging the certain levels can be set to exit the DRS recipe when they are used. They are defined in \definevariable{exit\_levels} and exiting python is controlled via \definevariable{exit} and \definevariable{Logger exit type}.
...
\noindent For this reason, and for convenience for passing between functions and recipes, a new object, based on a dictionary has been defined to handle all variables defined throughout the DRS. This is the parameter dictionary (\ParamDict) class (defined in \definevariable{\spirouConfig}). \\
...
As mentioned above in section \ref{ch:rules:drs_specific:param_dict} it is important to handle errors caused by variable definition. Included in the parameter dictionary definitions are a new set of exception handlers to be used with \ParamDict and the \definevariable{\spirouLog.logger} (aliased to \definevariable{WLOG} in most modules/recipes). It is very similar to standard python Exceptions but adds some new methods that can be accessed to be used with \definevariable{WLOG}. \\

An example is below of the \definevariable{ConfigError} exception (without using \ParamDict)
...
\end{pythonbox}
\vspace{0.5cm}
\noindent This functionality is coded into \ParamDict (with a \definevariable{WLOG} level set to `error') thus one only needs the following code:
\begin{pythonbox}
...
\end{cmdboxprint}
\begin{note}
Due to WLOG `error' currently meaning the code is exited a missing parameter will print the above message and then exit using the \definevariable{log\_exit\_type} exit strategy (see section \ref{ch:rules:drs_specific:logger}).
\end{note}